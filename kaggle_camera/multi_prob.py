...
def _oversample(images, crop_dims):
    ...
    h_indices = (0, (im_shape[0] - crop_dims[0])/2, im_shape[0] - crop_dims[0])
    w_indices = (0, (im_shape[1] - crop_dims[1])/2, im_shape[1] - crop_dims[1])
    crops_ix = np.empty((10, 4), dtype=int)
    curr = 0
    for i in h_indices:
        for j in w_indices:
            crops_ix[curr] = (i, j, i + crop_dims[0], j + crop_dims[1])
            curr += 1
    crops_ix[10] = np.tile(im_center, (1, 2)) + np.concatenate([
        -crop_dims / 2.0,
         crop_dims / 2.0
    ])
    ...
    # Extract crops
    crops = np.empty((20 * len(images), crop_dims[0], crop_dims[1],
    ...
    for im in images:
        for crop in crops_ix:
            ...
            ix += 1
        crops[ix-10:ix] = crops[ix-10:ix, :, ::-1, :]  # flip for mirrors
    return crops
...
def doClassify(model_def, model_weights, test_dir, test_txt, i):
    ...
    # print(pred)
    np.save('/mnt/lustre/yangkunlin/kaggle_camera/SE-ResNeXt-50/train_porb/prob_train_{}.npy'.format(i), pred)
    caffe.mpi_fin()
...
def main():
    model_def = myself + 'SE-ResNeXt-50_multi.prototxt'
    model_weights = '/mnt/lustre/yangkunlin/kaggle_camera/SE-ResNeXt-50/model_aug2-2_Adam/SE-ResNeXt-50_iter_194000.caffemodel'
    test_dir = '/mnt/lustre/yangkunlin/kaggle_camera/data'
    caffe.set_mode_gpu()
    model_def = model_def
    model_weights = model_weights
    ...
    for i in xrange(150):
        ...
        test_txt = '/mnt/lustre/yangkunlin/kaggle_camera/SE-ResNeXt-50/train3/'+'train_{}.txt'.format(i)
        # doClassify(model_def, model_weights, test_dir, test_txt, i)
        inputs = make_inputs(test_dir, test_txt)
        # print("inputs type is: {}".format(type(inputs[0])))
        pred = predictor.predict(inputs, oversample=True)
        # print(pred)
        np.save('/mnt/lustre/yangkunlin/kaggle_camera/SE-ResNeXt-50/train_prob2/prob_train_{}.npy'.format(i), pred)
    caffe.mpi_fin()
